...
至少两轮完整的"直觉先立起→当场被推翻"，双栏对照面板（左栏直觉粉 vs
右栏理性青）是唯一论证图式；每轮反差后立即解释机制，不欠账；结尾给
...
COLOR_SCHEME_NAME = "反差心理学·淡紫认知局"
COLOR_SCHEME_DESCRIPTION = (
    "淡紫白画布 + 直觉粉/理性青对照局 + 错误红破点/结论绿收尾 + 无衬线粗体字体，"
    "复用平台最早的认知紫默认色系语义映射，与其它风格族的暖白/暖纸背景形成区隔"
)
...
〇、画面规格与画布
横屏 16:9。内容画布淡紫白 #F7F3FF（复用平台最早的"认知紫"默认色系，
该色系本就标注"适合心理学/思维类内容"，比其它风格族的暖白/暖纸背景
更冷调、更清新）。亮底上的文字用深紫黑 #1C1433；辅助注释用浅紫灰
#8E7DC0。全片背景不变，无黑场，情感落差由节奏与色彩语义承担。
...
- 浅紫灰 #8E7DC0 + 低透明：已完成使命、退场前的过渡态（不超过 0.3
  秒，不作为常驻状态使用）
...
STYLE_TEMPLATE_DESCRIPTION = (
    "反差心理学的重构版风格族：淡紫白 + 直觉粉/理性青对照局视觉 + "
    "至少两轮「立-破-释」反差循环叙事（押注钩子/证据菜单/机制段/"
    "工具化纠偏收尾）+ 5.5-6字/秒打脸节奏 + 双轮翻案三段循环骨架 + "
    "当庭翻案增量动画。适合心理学、行为经济学类反直觉现象知识选题。"
)
