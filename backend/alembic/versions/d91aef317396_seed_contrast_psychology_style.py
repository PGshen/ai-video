...
至少两轮完整的"直觉先立起→当场被推翻"，双栏对照面板（左栏莓红 vs
右栏鼠尾草绿）是唯一论证图式；每轮反差后立即解释机制，不欠账；结尾给
...
COLOR_SCHEME_NAME = "反差心理学·莓绿判词局"
COLOR_SCHEME_DESCRIPTION = (
    "云杉白画布 + 莓红/鼠尾草绿对照局 + 深莓红破点 + 衬线体（宋体）+ "
    "直角矩形图形语言，与其它风格族的暖白/暖纸/淡紫背景及圆角卡片形成区隔"
)
...
〇、画面规格与画布
横屏 16:9。内容画布云杉白 #F5F6F2（带极淡的灰绿底色，呼应鼠尾草绿而
不抢戏，区别于其它风格族的暖白/暖纸/淡紫背景）。亮底上的文字用墨黑
#24261F；辅助注释用灰橄榄 #9A9C8C。全片背景不变，无黑场，情感落差由
节奏与色彩语义承担。

一、字体（本风格的身份标识）
全片一律衬线体（宋体/明朝体系），标题、标注与数字皆用衬线，标题短促
有力（≤12 字）；关键数字用大号衬线等宽数字；"破"句可用加粗或色块
高亮，不使用手写体或装饰字体。（与概念传记同属衬线家族，但字重更
硬朗、标题更短促，区隔交由色彩与图形语言承担，不再靠字体本身。）

二、语义色（含义锁定，全片唯一色彩系统，不得挪用）
- 莓红 #B23A5E：直觉方案、"我以为"、尚未验证的判断——不代表"错误"，
  只代表"未经检验"
- 鼠尾草绿 #4F7A5B：证据/真相、"实际上"、被验证过的结论，同时是
  机制解释段因果箭头链、圈注的强调色
- 深莓红 #7A2340：仅用于"破"瞬间的划叉/否决动作本身，是莓红的加深版
  而非新色相，不作常驻色，保持较高饱和度以保住"啪"的冲击力
- 灰橄榄 #9A9C8C + 低透明：已完成使命、退场前的过渡态（不超过 0.3
  秒，不作为常驻状态使用）
单屏语义色不超过 2 个色相家族（莓红系 + 鼠尾草绿系），色彩承载语义，
禁止纯装饰性用色，不再额外引入第三个不相关色相（如橙、紫、青）。

三、图形语言（硬性要求，防止圆角矩形泛滥）
- 默认容器一律用直角矩形（border_radius=0）或极小圆角（≤4px，视觉上
  近乎直角），不把"大圆角卡片"当成文本容器的默认形态
- 同屏允许出现的大圆角矩形不超过 1 个（仅收尾定格的方法条可用）
- 需要容器感但不想用卡片时，优先用：细线分割的直角矩形分栏、
  "标签文字 + 细线下划线"、直角矩形边框 + 顶部窄标题条
- 圆形是本风格另一种基础形（❶❷编号徽标、圈注强调），不受"圆角矩形"
  限制——圆形与"大圆角矩形卡片"是两种不同的形状语言，不要混同

四、核心图式：双栏对照面板（全片唯一论证图式，格式锁定）
一个直角矩形大框，中间一条细竖线分隔为左右两栏（不是两个独立的圆角
卡片并排）；左栏 = 直觉（"我以为"），莓红描边；右栏 = 真相
（"实际上"），鼠尾草绿描边；面板顶部各配 ≤6 字短标题；"破"瞬间右栏
内容以证据形式逐步显现，左栏同步被深莓红划叉或整体转为灰橄榄随即
退场（不得长期停留在灰态）。全片同一面板格式复用 2-3 次（对应两轮
反差 + 可选的开场下注呈现），格式不变、内容随轮次更换。

五、图示词汇表（按论证步骤选型，图标只作点缀，不作为镜头主体）
- 观众下注 → 二选一选项条（直角矩形分栏，非圆角按钮）/ 数轴刻度估算条
- 直觉支撑证据 → 简笔场景插画 + 数据标注
- 反证数据 → 散点/柱状/序列图，鼠尾草绿主色
- 机制解释 → 因果箭头链（大脑图标 → 启发式标签 → 偏差结果），
  关键节点用鼠尾草绿圆形圈注强调
- 纠偏方法 → 单条自检文字条（直角边框 + 顶部窄标题）/ 条件动作对
  （❶❷两个圆形编号，不超过两步），鼠尾草绿描边
...
呈现一个核心图示，容器形状遵循上述"图形语言"约束。

自检：一章内出现两个互相争夺注意力的主图示、双栏面板格式与前几次
不一致、或同屏出现超过 1 个大圆角矩形，即为不合格构图。"""
...
一、核心动作词汇表
- 立：直觉方案以莓红描边生长入场，配支撑证据逐条点出（0.15-0.2 秒
  间隔），让它"站稳"至少 1-2 秒
- 破：单一干脆动作二选一——
  1) 莓红内容整体翻转（Flip）露出鼠尾草绿背面的真相，0.4-0.5 秒
  2) 莓红内容被深莓红✗划叉（0.3 秒）后立即 FadeOut（不经过变灰过渡），
     鼠尾草绿真相同步从右侧生长入场
  破的瞬间独占一拍，前后各留 0.3-0.5 秒静止，不与其他动作叠加
- 释：机制因果链逐节点出（0.2 秒间隔），节点间箭头生长 0.3 秒
- 收尾方法条：淡入 + 轻微上浮 0.3 秒，自检问句逐字浮现
  ...
  "scenes": [
    {
      ...
      "description": "画布中央十枚硬币正面朝上依次点出排成一列，莓红描边二选一选项条（正面/反面）滑入下方悬停待选。退场：无（首章）。保留：十枚硬币序列（下一章反差的锚点）。",
      ...
      "beats": [
        ...
        },
        {
          ...
          "visual_action": "莓红描边二选一选项条（正面/反面）从下方滑入并轻微悬停。",
          "emphasis": "你猜",
          "transition": "continue",
          "fallback_weight": 1.0
        },
        {
          ...
          "visual_action": "反面选项条描边加粗并轻微放大一次，作为观众默认选择的视觉确认。",
          "emphasis": "该轮到反面了",
          "transition": "continue",
          "fallback_weight": 1.0
        }
    ...
    },
    {
      ...
      "description": "双栏对照面板首次出现（直角矩形，细竖线分隔左右两栏）：左栏莓红「该轮到反面了」承接上一章选项条，右栏鼠尾草绿「50% vs 50%」概率条同步生长；左栏被深莓红✗划叉后立即淡出。退场：左栏直觉选项条（划叉后0.3秒内淡出）。保留：右栏50/50概率条（本轮结论，下一章机制解释的锚点）。",
      "estimated_duration_seconds": 11.0,
      "beats": [
        {
          "beat_index": 0,
          "cue_text": "但硬币没有记忆。",
          "visual_action": "双栏面板顶部标题「我以为」「实际上」同步淡入生长，左栏继承上一章选项条。",
          "emphasis": "没有记忆",
          "transition": "continue",
          "fallback_weight": 1.0
        },
        {
          ...
          "visual_action": "深莓红✗从左上到右下划过左栏选项条，0.3秒内左栏整体淡出。",
          "emphasis": "没有任何影响",
        ...
        },
        {
          ...
          "visual_action": "右栏鼠尾草绿50%/50%概率条从中线向两侧生长，到达终点各轻微过冲一次定格。",
          "emphasis": "50%对50%",
          "transition": "continue",
          "fallback_weight": 1.0
        }
...
STYLE_TEMPLATE_DESCRIPTION = (
    "反差心理学的重构版风格族：云杉白 + 莓红/鼠尾草绿对照局视觉（衬线体、"
    "直角矩形图形语言）+ 至少两轮「立-破-释」反差循环叙事（押注钩子/"
    "证据菜单/机制段/工具化纠偏收尾）+ 5.5-6字/秒打脸节奏 + 双轮翻案"
    "三段循环骨架 + 当庭翻案增量动画。适合心理学、行为经济学类"
    "反直觉现象知识选题。"
)
