# 记忆唤醒·理科卡片 风格家族 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 新增「记忆唤醒·理科卡片」风格家族到数据库，包含 5 个 prompt_components seed 和 1 个 style_templates bundle，面向初高中理科复习短视频（9:16 竖屏，Manim 引擎）。

**Architecture:** 单个 Alembic migration 文件，同时写入 `prompt_components`（5 条）和 `style_templates`（1 条）。与 `b3f1d6a8c2e5_seed_ensemble_theater_style.py` 模式完全一致：先 bulk_insert components，再 bulk_insert template；downgrade 按反序删除。

**Tech Stack:** Python, Alembic, SQLAlchemy (Core bulk_insert), PostgreSQL/JSONB

## Global Constraints

- Manim 引擎，9:16 竖屏（1080×1920），`config.frame_height=16, config.pixel_height=1920, config.pixel_width=1080`
- UUID 前缀规则：`b7a2f8c1-1004-4000-8000-00000000000X`（1-5 为组件，ff 为模板），不得与现有前缀 1001/1002/1003 冲突
- `down_revision` 必须指向当前 head：`a4d6b7e9c102`
- revision ID 使用：`d4e5f6a1b2c3`
- 所有 `prompt_text` 必须用中文编写，与现有风格的格式对齐（`【类别：风格名】` 标题 + 分节规则）
- 文件命名：`backend/alembic/versions/d4e5f6a1b2c3_seed_memory_recall_science_card_style.py`

---

### Task 1: 编写并运行 Alembic migration

**Files:**
- Create: `backend/alembic/versions/d4e5f6a1b2c3_seed_memory_recall_science_card_style.py`

**Interfaces:**
- Produces: 5 个 `prompt_components` 记录 + 1 个 `style_templates` 记录写入数据库
- `style_config` JSONB 结构：`{"color_scheme": "<uuid>", "narrative_style": "<uuid>", "pacing": "<uuid>", "scene_structure": "<uuid>", "animation_style": "<uuid>"}`

- [ ] **Step 1: 创建 migration 文件**

创建 `backend/alembic/versions/d4e5f6a1b2c3_seed_memory_recall_science_card_style.py`，内容如下：
...
```bash
cd backend && /Users/peng/.local/bin/uv run pytest tests/test_style_templates.py -v
```

预期输出：所有测试 PASS（migration 文件不影响单元测试，因为测试用 mock_db）

- [ ] **Step 3: 运行 alembic upgrade 确认 migration 可执行**
...
预期输出：
```
INFO  [alembic.runtime.migration] Running upgrade a4d6b7e9c102 -> d4e5f6a1b2c3, seed_memory_recall_science_card_style
```

若报错 `Can't locate revision identified by 'a4d6b7e9c102'`：说明当前数据库 head 不是 `a4d6b7e9c102`，需先检查实际 head：
```bash
cd backend && /Users/peng/.local/bin/uv run alembic current
```
并相应修改 `down_revision`。
...
with sync_engine.connect() as conn:
    rows = conn.execute(text(\"SELECT name, category FROM prompt_components WHERE name LIKE '记忆唤醒%' ORDER BY category\")).fetchall()
    for r in rows:
        print(r)
    tmpl = conn.execute(text(\"SELECT name, style_config FROM style_templates WHERE name = '记忆唤醒·理科卡片'\")).fetchone()
    print(tmpl)
"
```
...
预期输出（5 个组件 + 1 个模板）：
```
('记忆唤醒·Manim图解动效', 'animation_style')
('记忆唤醒·白底明亮知识卡片', 'color_scheme')
('记忆唤醒·记忆钩子三段式', 'narrative_style')
('记忆唤醒·短视频竖屏节奏', 'pacing')
('记忆唤醒·知识卡片三层结构', 'scene_structure')
('记忆唤醒·理科卡片', {'animation_style': 'b7a2f8c1-1004-...', ...})
```

- [ ] **Step 5: 验证 downgrade 可执行**

```bash
cd backend && /Users/peng/.local/bin/uv run alembic downgrade -1
```

预期输出：`Running downgrade d4e5f6a1b2c3 -> a4d6b7e9c102`，无报错。
...
```bash
git add backend/alembic/versions/d4e5f6a1b2c3_seed_memory_recall_science_card_style.py
git commit -m "feat: add 记忆唤醒·理科卡片 style family (5 components + bundle template)"
```
